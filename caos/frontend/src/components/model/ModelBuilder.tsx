...
function WorksheetGrid({ tab, selected, onSelect }: { tab: WorksheetTab; selected: WorksheetCell | null; onSelect: (cell: WorksheetCell) => void }) {
  ...
  return <div className="worksheet-scroll" tabIndex={0} role="region" aria-label={`${tab.title} worksheet; scroll horizontally and vertically`}>
    <p className="worksheet-authority-note">Historical and calculated cells are locked. Change forward inputs in Assumptions; previews recalculate forecast periods only.</p>
    <table className="worksheet-grid">
      ...
      <tbody>{Array.from({ length: tab.max_row }, (_, rowIndex) => {
        ...
        return <tr key={row}><th scope="row">{row}</th>{Array.from({ length: tab.max_column }, (_unused, columnIndex) => {
          ...
          const className = ["worksheet-cell", authority.className, fillClass, cell?.style.bold ? "is-bold" : "", cell?.style.italic ? "is-italic" : "", cell?.value_type === "number" || cell?.value_type === "formula" && typeof cell.value === "number" ? "num" : "", selected?.address === cell?.address ? "is-selected" : ""].filter(Boolean).join(" ");
          ...
          return <td key={column} className={className} title={cell?.formula ? `${authority.label} · ${cell.formula}` : authority.label} data-write-class={cell?.write_class || "LOCKED"} data-address={address} data-row={row} data-column={column} tabIndex={activeCell.row === row && activeCell.column === column ? 0 : -1} aria-label={`${address}: ${value === "" ? "blank" : value}. ${authority.label}.${hasLineage ? " Press Enter to show lineage." : ""}`} onFocus={() => setActiveCell({ row, column })} onKeyDown={(event) => onCellKeyDown(event, row, column, cell)}>{hasLineage && cell ? <button className="worksheet-cell-link" type="button" tabIndex={-1} aria-label={`Show lineage for ${cell.semantic_id || cell.address}`} aria-controls="model-cell-lineage" aria-expanded={selected?.address === cell.address} onClick={() => { setActiveCell({ row, column }); onSelect(cell); }}>{value || "—"}</button> : value}</td>;
        })}</tr>;
      })}</tbody>
    </table>
  </div>;
...
type ModelRevision = Omit<ModelPreview, "draft_generation" | "deltas"> & {
  ...
  // The revision envelope is service-owned and names these `created_by` /
  // `created_at`; a revision is created by being signed. Reading invented
  // `signed_*` keys rendered "undefined · —" under every Active Analyst Model.
  created_by: string;
  created_at: string;
  state: "ACTIVE" | "SUPERSEDED" | "STALE";
  // Optional on purpose: a build or revision that has never been exported
  // carries no export record, and a missing one must degrade its own control
  // rather than throw out of the render and blank the workspace.
...
export default function ModelBuilder({
  ...
  const [unavailable, setUnavailable] = useState<{ oneWay?: boolean; rebase?: boolean; revisionExport?: boolean; revisionDownload?: boolean }>({ oneWay: true });
  const [loading, setLoading] = useState(true);
  const [loadError, setLoadError] = useState("");
  const [pending, setPending] = useState("");
  const [message, setMessage] = useState("");
  ...
  const runOneWay = async () => {
    if (!canWrite || !build || !registry || !selectedDefinition || !sensitivityScope.editable) return;
    const action = ++actionGeneration.current;
    const expectedAuthority = authorityKey.current;
  ...
  const runScenario = async () => {
    if (!canWrite || !build || !registry) return;
  ...
  return <div className="grid model-builder analyst-model-builder">
    <section className="panel span-12 model-builder-command">
      <div className="panel-header"><div><p className="eyebrow">{activeRevision ? "SIGNED ANALYST AUTHORITY" : "ANALYST MODEL WORKSPACE"}</p><h2>Model Builder</h2></div><span className={`status ${modelStatusTone(status)}`} role="status" aria-live="polite">{status?.replaceAll("_", " ")}</span></div>
      <div className="panel-body model-builder-command-body"><div><strong>{activeRevision ? `Active Analyst Model · R${activeRevision.revision_number}` : "No Active Analyst Model"}</strong><p className="muted">{activeRevision ? `${activeRevision.created_by} · ${formatDate(activeRevision.created_at)} · ${activeRevision.note}` : status === "READY" ? "Application Model Build is available for comparison. Sign-off stores only the exact revision you approve." : status === "QUEUED" || status === "BUILDING" ? "The Application Model Build is still calculating." : "No ready Application Model Build is available for comparison."}</p></div><div className="model-primary-action">{renderPrimaryAction()}<button className="button small" type="button" disabled={loading} onClick={() => void refresh()}>{loading ? "Refreshing…" : "Refresh authority"}</button></div></div>
