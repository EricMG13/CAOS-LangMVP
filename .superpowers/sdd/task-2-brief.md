...
- The application has no executable external filing-fetch runtime. Four CP-4 vendored methodology files still instruct an agent to search, fetch, and vault external filings. Those instructions conflict with the required real-user journey: source bytes enter only through user upload.
...
2. Delete the complete CP-4 external acquisition reference section and all references to it from the four deployed files. Where a governing document is absent, require supplied governing documents or a typed evidence gap/blocked result. Never suggest web search, retrieval, fetching, vaulting, or substitution with an external snippet.
...
Also run a repository search matching the prohibition test. Expected: zero forbidden acquisition references in product/runtime/methodology/test-fixture surfaces; bundle verification and independent byte pin both pass; 26 modules report zero drift; no integrity file changes on a second regeneration.
