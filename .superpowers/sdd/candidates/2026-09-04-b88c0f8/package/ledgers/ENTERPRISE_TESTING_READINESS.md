...
| ETR-B06 | Closed by Task 12b (2026-09-03) and rerun on candidate `2026-09-04-b88c0f8` (2026-09-04: pip-audit, bandit, npm audit, gitleaks, Trivy fixable gate and the SBOMs bound to both image ids — `gates/scans/scan-manifest.json`; `test_workflow_security` 22 and `test_recorded_review` 8 passed in the candidate junit): the AI review is replaced by a recorded, read-only diff review (`security-review.yml` → `caos/scripts/recorded_review.py`, `contents: read`, no secret, no network) whose record is retained on every pull request and whose non-vacuity is tested (`caos/tests/test_recorded_review.py`); every workflow is pinned, read-only and free of event text in shell (`caos/tests/test_workflow_security.py`); the scanners prove their scope and bind their reports to the image digests with an SBOM (`caos/scripts/scan_floors.py`); ledger row F-OPS-10 is PASS | Keep the recorded review as the pull-request check and the workflow tests in the suite; rerun the scans against the frozen candidate images in Task 13 |
| ETR-B07 | Closed by Task 12a (2026-09-03) and rerun on candidate `2026-09-04-b88c0f8` (2026-09-04: `test_postgres_races` 27 passed against the pinned PostgreSQL container — `gates/pytest-backend-junit.xml`): `caos/tests/test_postgres_races.py` runs the ingestion, source-set, withdrawal, assumption, loan-universe, event-sequence, budget, acceptance, build claim and queue, model sign-off, opinion, draft, freeze, publication and filing races on two independent PostgreSQL connections with forced interleavings, in CI against the digest-pinned container and locally against the QA container; six store defects it found were repaired (`SPEC_RECONCILIATION.md`, "Two-connection PostgreSQL races"; `docs/DECISIONS.md` §14.21) | Keep the module in the `postgres` CI job with `CAOS_REQUIRE_POSTGRES=1`; rerun it against the frozen candidate in Task 13 |
| ETR-B08 | Closed on candidate `2026-09-04-b88c0f8` (2026-09-04): the release gate is OpenAPI discovery — `run_sec_audit.py` walks every served route (59 routes, 507 matrix cells, 0 failures) and refuses to run if OpenAPI and the registered routes differ; the a11y sweep scans every UI destination (9 routes); the package retains the OpenAPI document and route inventory (`package/inventory/`). `production-inventory.mjs` is not a gate and not in CI | Keep OpenAPI discovery as the inventory gate |
| ETR-B09 | Closed by exclusion on candidate `2026-09-04-b88c0f8` (2026-09-04): OpenRouter is excluded from the enterprise profile (development-only, refused in production; `test_openrouter_provider` margin tests in the candidate junit) | Shape proof across every model and maximum request shape before any later admission |
| ETR-B10 | Closed by Task 12a (2026-09-03) and rerun on candidate `2026-09-04-b88c0f8` (2026-09-04: `test_single_instance` 49 passed, SIM-001–030 30/30 with 67 retained tests — `gates/sim-evidence.json`; second instance refused `INSTANCE_ALREADY_RUNNING` on the frozen image, kill-and-recover in 11 s with counts unchanged, backup and restore drill passed — `gates/stack/stack-gates.txt`; the eight-hour soak completed but PERF-013/014/015 stay open in the package): the ceiling is recorded in `caos/deploy/ENVIRONMENT_MANIFEST.md` and declared in Compose (`deploy.replicas: 1`); the app takes an exclusive OS lock over the checkpoint location before it recovers or serves (`INSTANCE_ALREADY_RUNNING`, proven by a second process in `test_single_instance.py`) beside the PostgreSQL role locks; SIM-001–SIM-030 are retained in `docs/SIMULATION_LEDGER.csv`, including checkpoint absent/truncated/corrupt/locked and the restart loop | Rerun the single-instance and simulation evidence against the frozen candidate in Task 13; the soak and saturation faults remain candidate-only |
| ETR-B11 | OPEN on candidate `2026-09-04-b88c0f8` (2026-09-04): Distressed and Deep Research execute through the ordinary provider path under host control (corpus host control 35 passed, Deep Research through the plan gate; the Deep Research golden journey through opinion, freeze, filing, receipt and offline verification was rehearsed on a dev server at the commit, not on the image) — neither is live-model qualified and C21/C22 are external | Pass the same run, corpus, model, publication, and audit matrix as every other pathway against the qualified provider; the Deep Research cell also needs the question-specific C22 pack |
| ETR-B12 | Closed by Task 9 (2026-09-02) under host control and rerun on candidate `2026-09-04-b88c0f8` (2026-09-04: `test_source_complete_modelling_spec` 27 passed; CALC-001–020 all mapped to passed candidate tests in the package): every build carries a source-lineage record (intake disposition and reason, expected consumers, citing artifacts, model-facing tables, one binding per source), a `used` relevant document bound to nothing is the typed `MODEL_SOURCE_LINEAGE_INCOMPLETE` and never READY, and every pathway declares one model effect on the nearest validated Full Credit model (`docs/DECISIONS.md` §14.18). The metamorphic cases in `spec/test_source_complete_modelling_spec.py` and the six-route corpus control assert the answer-keyed changes. Live-model qualification of every effect remains open | Keep the lineage oracle, the metamorphic cases and the corpus lineage assertions retained; qualify every pathway's effect against the qualified provider in Tasks 11 and 13; the licensed market-marks pack for Relative Value is an external input |
| ETR-B13 | OPEN on candidate `2026-09-04-b88c0f8` (2026-09-04): the cross-format goldens and the three-engine journey are retained, but the benchmark set is not pinned and REV-006 is outstanding | Pin the Credit Operating System reference and pass automated rendering checks plus independent side-by-side review for every pathway and published format |
