...
|---|---|---|
| ETR-B01 | The current user journey requires case creation, pathway choice, compilation, run start, acceptance, and report setup | Add and test a document-first journey that creates or resolves the case, selects the default governed route, starts work, and opens the review result without analytical form filling |
...
| ETR-B11 | Distressed and Deep Research are exposed by current contracts/UI but are not engine-qualified | Implement their route and methodology contracts, then pass the same run, corpus, model, publication, and audit matrix as every other pathway |
| ETR-B12 | No candidate evidence proves every relevant supplied annual, quarterly/interim, and forecast document feeds analysis and the financial model | Produce a complete inclusion/exclusion manifest and trace every relevant file to model inputs, assumptions, calculations, or cited analysis |
