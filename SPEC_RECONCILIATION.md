...
| 7 | Model calculation is pure and finite — non-finite values and zero denominators refused, forecast values driver-sourced | `spec/test_model_builder_spec.py::test_finite_guards_reject_non_finite_and_zero_denominators` (production bundle) + `…::test_json_value_rejects_decimal_that_overflows_float` + `…::test_finite_operand_guard_names_the_offending_operand` | green |
| 8 | Budgets fail closed — every ceiling refuses before overspend; no provider call without a reservation; unresolved inflight fails the resumed run | `spec/test_budget_spec.py::test_each_ceiling_refuses_the_next_operation_before_overspend` (durable RunStore) + `…::test_no_provider_call_without_a_successful_reservation` + `…::test_unresolved_inflight_reservation_fails_closed_on_resume_without_respend` | green |
...
## User-uploaded issuer corpus qualification (2026-08-31)

The former mixed-source fixture list was replaced by one coherent Carnival
Corporation case containing 30 issuer-hosted PDFs: FY2023–FY2025 annual reports,
quarterly actuals, management guidance and forecasts, and an executed term-loan
agreement package. Every retained byte is SHA-256-pinned in `sources.txt`.
`fetch.sh` rejects an unapproved host, a non-PDF response, a missing document,
or a digest mismatch; neither the application nor pytest retrieves evidence.
Tests upload the retained files through `POST /api/cases/{case_id}/sources`, the
same multipart route used by a user.

Collected results:

- all 30 documents were admitted and produced extractable evidence blocks;
- `CORPUS_FULL=1` passed 34 tests, including all four executable pathways at
  Screen and Full depth, and typed refusal without pinning for the two route
  families whose engine graphs do not yet exist;
- the merged G4/G7 backend suite passed 630 tests with the corpus present (the two
  optional PostgreSQL integration tests also passed in a separate configured run);
- the host-control provider remains scripted. These results do not prove live
  model interpretation, complete source use, CP-MODEL construction, browser
  display, deliverable generation, or external publication.

The pack is therefore a valid performing-credit ingestion/host-control baseline,
not the all-pathway enterprise qualification corpus. Relative Value still needs
...
## Candidate 2026-09-04-b88c0f8 evidence package (2026-09-04, enterprise Task 13, ER-G10)

The release evidence package for candidate `2026-09-04-b88c0f8` (tag
`enterprise-candidate-2026-09-04`, commit `b88c0f8ca11af3200e8bb21daab16d838c64d39f`,
app image `sha256:10ec8aa0…`, worker image `sha256:526c2d5f…`) lives under
`.superpowers/sdd/candidates/2026-09-04-b88c0f8/package/`: `PACKAGE_MANIFEST.json`
(G0–G9, every check, every blocker, exclusions, test-only limitations, missing
artifacts, every object's sha256), `checks.csv` (the 340 checks of
`ENTERPRISE_TESTING_READINESS.md`, the 30 simulations among them), and
`PACKAGE.sha256` (the package digest). `assemble_package.py` builds it from
retained candidate artifacts only and `verify_evidence_package.py` (standard
library) re-verifies every object on a separate copy; both are in the package.
The narrative is `.superpowers/sdd/enterprise-task-13b-report.md`.

**The ten invariants on this candidate.** Every test the table above names —
26 tests across invariants 1–10, parametrized cells included — is retained as
`passed` in the candidate's own junit reports
(`gates/pytest-backend-junit.xml`, `gates/pytest-corpus-full-junit.xml`, backend
`1242 passed, 1 skipped`); the check was mechanical (test name against the
junit, not prose). Two references in the table use the `…::` shorthand across
modules: `test_run_pins_build_id_and_refuses_execution_under_a_different_bundle`
is in `spec/test_modules_spec.py` and `test_compiled_route_matches_its_pinned_golden`
in `test_bundle.py` (twelve cells). All ten invariants are green on the candidate.

**Check map.** 340 checks: 222 `PASS` (a retained passed test or artifact covers
the whole condition), 29 `PROVED HOST CONTROL` (only the answer-keyed
orchestration binding proves it; the live half is external), 17 `BLOCKED
EXTERNAL`, 71 `OPEN` with a named owner, 1 `FAIL` (PERF-009: `list_cases`
p95 1.28 s, `accept` p95 2.23 s at the declared profile on one instance).
`CALC-001–020` carry the table above verbatim, every cited test re-verified.
The other families' rows name what is proven and what is missing in
`checks.csv`; nothing was marked from prose, a skipped check, or another
candidate.

**What the package does not contain, and why the candidate cannot be
signed.** No live matrix cell (the ER-L3 log has no tick; the credential and
...
yields no READY Full Credit model); REV-001–REV-015, all prepared and none
returned; coverage reports; the benchmark set; the penetration test; the
signature. `PACKAGE_MANIFEST.json::signable` is `false` and
`not_signable_because` lists each reason.

### Anti-vacuity ledger

| Doubt | Check |
| --- | --- |
| A check could be marked PASS on a test that no longer exists or failed | `assemble_package.py` refuses any `junit:` citation absent from the candidate junit or not `passed`; `verify_evidence_package.py` repeats the check on the copy (`CITED_TEST_NOT_PASSED`) |
| The package could be altered after hashing | the verifier convicted a one-byte change (`OBJECT_DIGEST_MISMATCH`) and a removed object (`OBJECT_MISSING`) on the verification copy during ER-G10 |
...
The dated observations above remain historical. Current boundary, session,
manifest, startup and renderer rules are recorded in DECISIONS §14.28. The
invariant 7/8 rows now cite production calculation and durable ledger paths;
removed stand-alone test helpers are not invariant evidence. See
`audit-fixes-status.md` for the applied findings and exact verification limits.
