...
The review began on the G4 worktree rooted at commit `0d44b19cdece282498b7bcaadd7b5f533bf45f6d` and was revalidated after the concurrent G7 observability work merged. The cited line numbers describe the resulting local `main` files. The review treats uploaded bytes, browser requests, provider responses, internal network peers, the database, the backup destination, and document-derived text as potentially hostile. It trusts the host kernel, Docker daemon, and age implementation. A root or Docker-daemon compromise can bypass every in-container control and is outside this component model.
...
The provider receives the bounded source manifest and validated upstream Markdown in the initial user prompt (`caos/server/caos/engine/runtime.py:491-528`), then receives exact evidence text only after an allowed `read_evidence` call (`caos/server/caos/engine/loop.py:258-277`). Anthropic is selected first; OpenRouter is selected only when it is the sole configured key (`caos/server/run.py:25-37`). The supplied Compose deployment passes only the Anthropic key to the app (`caos/deploy/docker-compose.yml:58-60`), so OpenRouter is supported code but is not wired by this Compose file.

The requested worker to LibreOffice to XLSX boundary is not the current runtime flow. The worker calls `ModelService.run_export` (`caos/server/worker.py:52-64`), which calls `_render_bytes`, and `_render_bytes` invokes the Python `render_workbook` method (`caos/server/caos/models/service.py:917-923`). LibreOffice is installed and exercised by the image-build smoke through `CpModelBundle.export` (`caos/deploy/Dockerfile:30-37`; `caos/deploy/verify_image_resources.py:39-63`), and that separate path invokes `soffice` with a 90-second timeout and validates the recalculated workbook (`caos/server/caos/methodology/vendor/deploy_v/skills/cp-model/scripts/cp_model_v3/builder.py:119-157`; `caos/server/caos/methodology/vendor/deploy_v/skills/cp-model/scripts/cp_model_v3/builder.py:365-418`).
...
|---|---|---:|---:|---:|---|
| R-EDGE-1 | A compromised trusted peer can forge browser identity because the edge credential is a shared bearer secret. The worker receives that secret even though it does not serve HTTP (`caos/deploy/docker-compose.yml:84-94`), and the app accepts any matching secret plus forwarded identity (`caos/server/caos/identity.py:53-84`). | 3 | 5 | **15 High** | **Code and deployment-config fix:** give the worker a role-specific settings validator, remove `EDGE_PROXY_SECRET` and `SESSION_SECRET` from it, and replace the shared bearer assertion with authenticated service identity or a private app listener. |
...
| R-PROV-1 | The provider receives confidential case metadata, upstream analyses, and requested evidence text. The repository contains no enforceable retention, training, residency, subcontractor, or deletion control for that third party (`caos/server/caos/engine/authority.py:71-100`; `caos/server/caos/engine/loop.py:258-277`). | 5 | 5 | **25 High** | **Documented accepted risk:** require an approved enterprise provider contract and data processing agreement, record residency and retention choices, and offer a private or self-hosted provider when that acceptance is not available. |
| R-PROV-2 | Provider peer identity relies on normal HTTPS and the platform trust store. The code has no egress allowlist, private endpoint, certificate pin, or proxy policy (`caos/server/caos/engine/openrouter.py:53-93`; `caos/server/caos/engine/anthropic.py:28-35`). | 2 | 5 | 10 Medium | Deployment-config fix: restrict egress to approved provider endpoints and use a controlled egress proxy or private endpoint. |
| R-PROV-3 | One timeout retry and typed failure bound provider outages, but no alternate provider failover exists in one run. Anthropic always wins when both keys are configured (`caos/server/run.py:25-37`; `caos/server/caos/engine/loop.py:180-234`). | 4 | 3 | 12 Medium | Document the availability objective and re-run policy; add failover only if procurement requires it without changing a run's pinned provider identity. |
| R-PROV-4 | OpenRouter reserves input budget from a local tokenizer estimate with a fixed 1.5 margin, not an authoritative count (`caos/server/caos/engine/openrouter.py:53-69`; `caos/server/caos/engine/openrouter.py:184-190`). I8 still controls aggregate reconciliation and future calls, but it does not prove that this pre-call estimate is exact. | 2 | 4 | 8 Medium | Code or accepted-risk decision: calibrate continuously and fail closed on a larger bound, or restrict production to Anthropic's count endpoint. |
| R-PROV-5 | The attempt ledger retains request digest, request ID, usage, and stop reason, but intentionally does not retain provider response content. That protects confidentiality but limits later non-repudiation of provider behavior (`caos/server/caos/engine/loop.py:236-246`; `caos/server/caos/engine/runtime.py:541-555`). | 2 | 3 | 6 Low | Document the forensic limitation; do not add raw prompt logging without a separate encrypted evidence-retention design. |
| R-WRK-1 | The production export worker does not call the LibreOffice recalculation and validation path. It renders with Python and publishes those bytes, while only the image-build fixture exercises LibreOffice (`caos/server/caos/models/service.py:917-936`; `caos/deploy/verify_image_resources.py:39-63`). | 4 | 5 | **20 High** | **Code fix:** route production build and revision exports through the existing `CpModelBundle.export` and `export_revision` path, then preserve the current hash-addressed publish and download verification. |
| R-WRK-2 | The worker runs document and workbook code with read/write access to the entire vault and the shared DB credential, and it also receives unused edge, session, ClamAV, and Anthropic secrets (`caos/deploy/docker-compose.yml:74-101`; `caos/server/worker.py:68-79`). A worker or LibreOffice compromise therefore reaches unrelated cases and can forge edge identity. | 3 | 5 | **15 High** | **Code and deployment-config fix:** split worker settings, remove unused secrets, use a least-privilege DB role, and stage one export into a job-scoped directory or broker instead of mounting the whole vault read/write. |
| R-WRK-3 | Export jobs have no claim transition before rendering. The PostgreSQL singleton watchdog refuses normal duplicate workers and terminates on lock-session loss, but an executor that bypasses that deployment guard or overlaps its five-second detection interval can still render the same queued export; only the final failure update is conditional (`caos/server/caos/storage/store.py:163-165`; `caos/server/caos/storage/store.py:189-244`; `caos/server/worker.py:52-64`; `caos/server/caos/models/service.py:925-968`). | 2 | 3 | 6 Low | Code fix: add a compare-and-swap export claim with an execution token before any render. |
...
| Elevation of privilege | Source-derived values stay in an explicitly untrusted user prompt, and the system prompt is verified methodology only (`caos/server/caos/engine/authority.py:71-100`). Provider identity is rebuilt, tool access is limited to one strict evidence tool, and citations must equal delivered evidence (`caos/server/caos/engine/provider.py:15-28`; `caos/server/caos/engine/runtime.py:579-596`). Loan-workbook display text has a separate Unicode rendering gap, R-ING-4. | I1/I2: pin and host evidence boundary tests above. I3: `test_host_owns_identity_and_discards_provider_frontmatter` (`caos/tests/spec/test_modules_spec.py:121`). I4: `test_verify_at_use_rejects_bytes_that_mismatch_the_pinned_manifest` (`caos/tests/spec/test_modules_spec.py:81`). I9: `test_canonical_output_schema_is_strict_and_bounded` (`caos/tests/spec/test_modules_spec.py:108`). None for R-ING-4. |

### App to provider

| STRIDE | Threat and current control | Invariant reconciliation |
|---|---|---|
| Spoofing | A provider can claim a forged module, run, build, or source identity in output. The host discards and rebuilds those fields (`caos/server/caos/engine/authority.py:25-34`; `caos/server/caos/engine/runtime.py:579-596`). Network-level provider identity still depends on ordinary HTTPS, R-PROV-2. | I3: `test_host_owns_identity_and_discards_provider_frontmatter` (`caos/tests/spec/test_modules_spec.py:121`). None for network peer policy. |
| Tampering | The provider can return undeclared fields, malformed usage, forged citations, unexpected tools, or bad stop reasons. Host parsing and canonicalization refuse each class (`caos/server/caos/engine/loop.py:35-56`; `caos/server/caos/engine/loop.py:90-113`; `caos/server/caos/engine/loop.py:236-308`). | I2: `test_read_outside_pinned_source_set_fails_closed` (`caos/tests/spec/test_evidence_spec.py:32`). I3: host identity test above. I9: `test_canonical_output_schema_is_strict_and_bounded` and `test_forged_citation_outside_delivered_set_is_rejected_at_canonicalization` (`caos/tests/spec/test_modules_spec.py:108`; `caos/tests/spec/test_evidence_spec.py:125`). |
| Repudiation | The host records a request digest, provider request ID, usage, and stop reason after interaction (`caos/server/caos/engine/loop.py:236-246`). Raw response content is intentionally not retained, R-PROV-5. | I8: `test_no_provider_call_without_a_successful_reservation` proves the durable pre-call reservation (`caos/tests/spec/test_budget_spec.py:77`). It does not prove provider non-repudiation. |
...
| Denial of service | Provider slots, per-call timeout, one byte-identical retry, token budgets, and fail-closed unresolved reservations bound resource use (`caos/server/caos/engine/loop.py:59-77`; `caos/server/caos/engine/loop.py:156-246`). Outage still fails a run, R-PROV-3; OpenRouter's estimate remains R-PROV-4. | I8: `test_each_ceiling_refuses_the_next_operation_before_overspend`, `test_no_provider_call_without_a_successful_reservation`, and `test_unresolved_inflight_reservation_fails_closed_on_resume_without_respend` (`caos/tests/spec/test_budget_spec.py:126`; `caos/tests/spec/test_budget_spec.py:77`; `caos/tests/spec/test_budget_spec.py:88`). None proves external availability or estimate exactness. |
...
| Information disclosure | The worker runs non-root with all Linux capabilities dropped and a read-only root filesystem (`caos/deploy/docker-compose.yml:95-101`). Failures log only the exception class in a bounded structured event (`caos/server/worker.py:29-64`), but the worker still mounts the whole vault read/write and receives unrelated secrets, R-WRK-2. | None. |
...
| Elevation of privilege | A LibreOffice or worker compromise executes as the `caos` user without capabilities, but that user still owns the job's DB and vault access plus edge and provider secrets, R-WRK-2. | None. |
